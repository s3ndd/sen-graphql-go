...
type SiteLoaderConfig struct {
	...
	Fetch func(keys []string) ([]*model.Site, []error)
...
func (l *SiteLoader) Load(key string) (*model.Site, error) {
...
func (l *SiteLoader) LoadThunk(key string) func() (*model.Site, error) {
	l.mu.Lock()
	if it, ok := l.cache[key]; ok {
		l.mu.Unlock()
		return func() (*model.Site, error) {
			return it, nil
		}
	}
	if l.batch == nil {
...
func (l *SiteLoader) LoadAll(keys []string) ([]*model.Site, []error) {
	results := make([]func() (*model.Site, error), len(keys))

	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}

	sites := make([]*model.Site, len(keys))
...
// This method should be used if you want one goroutine to make requests to many
// different data loaders without blocking until the thunk is called.
func (l *SiteLoader) LoadAllThunk(keys []string) func() ([]*model.Site, []error) {
	results := make([]func() (*model.Site, error), len(keys))
	for i, key := range keys {
		results[i] = l.LoadThunk(key)
	}
...
func (l *SiteLoader) Prime(key string, value *model.Site) bool {
	l.mu.Lock()
	var found bool
...
func (l *SiteLoader) Clear(key string) {
	l.mu.Lock()
	delete(l.cache, key)
	l.mu.Unlock()
}

func (l *SiteLoader) unsafeSet(key string, value *model.Site) {
	if l.cache == nil {
		l.cache = map[string]*model.Site{}
	}
	l.cache[key] = value
}
...
func (b *siteLoaderBatch) keyIndex(l *SiteLoader, key string) int {
